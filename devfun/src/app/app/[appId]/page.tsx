...
import { DevFunAppDetail } from '@/types/devfun';
  ...
  useEffect(() => {
    const fetchApp = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const data = await DevFunApiService.fetchAppById(appId);
        setApp(data);
      } catch (err) {
    ...
    <div className="min-h-screen bg-black text-white relative">
      ...
      <div className="fixed top-20 right-8 z-50">
        <div className="inline-flex items-center gap-2 bg-black/80 backdrop-blur-sm text-pink-300 px-4 shadow-lg">
          ...
          <span className="text-lg font-bold">{app.likeCount.toLocaleString()} likes</span>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[160px_1fr] gap-8 mb-10 pt-16">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="text-gray-400 text-md leading-relaxed">{app.description}</div>
            <div className="mt-5 flex gap-3 flex-wrap">
              ...
              {app.twitter && (
                <a href={app.twitter} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-black/60 hover:border-gray-600 text-gray-200 px-4 py-2 rounded-none">
              ...
              {app.telegram && (
                <a href={app.telegram} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-black/60 hover:border-gray-600 text-cyan-400 px-4 py-2 rounded-none">
              ...
              <div className="inline-flex items-center gap-2 bg-black/60 text-2xl text-yellow-200 px-4 py-2 rounded-none">
                ...
                <span className="text-2xl">{app.runsCount.toLocaleString()} runs</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Launch Button */}
        <div className="mb-12">
          <button className="bg-gradient-to-r from-green-400 to-green-300 hover:from-green-500 hover:to-green-400 text-black px-8 py-4 font-extrabold text-xl rounded-none">
            Launch App
          </button>
        </div>
      </div>
