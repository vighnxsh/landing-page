...
type ClaimPartnerFeesOptions = {
  onSuccess?: (signature: string) => void;
  onError?: (error: string) => void;
};

type ClaimPartnerFeesParams = {
  pool: string;
...
export function useClaimPartnerFees() {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [signature, setSignature] = useState<string | null>(null);
  const { publicKey, signTransaction } = useWallet();

  const claimPartnerFees = async (
    params: ClaimPartnerFeesParams,
    options: ClaimPartnerFeesOptions = {}
  ) => {
    ...
    try {
      ...
      const response = await fetch('/api/claim-partner-fees', {
        method: 'POST',
        headers: {
      ...
      setError(error);
      toast.error(`Failed to claim partner fees: ${errorMessage}`);
