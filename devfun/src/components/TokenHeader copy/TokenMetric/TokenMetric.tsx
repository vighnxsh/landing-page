...
export const TokenMetric: React.FC<TokenMetricProps> = ({
  ...
  return (
    <div className={className}>
      <div className="truncate text-3xl  leading-none text-pink-500">{label}</div>
      {children !== undefined ? (
        children
      ) : (
...
export const MetricMcap: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    ...
    <TokenMetric
      className={cn('flex py-3 text-xl items-center justify-between', className)}
      label="Mkt Cap"
      num={baseAsset?.mcap ?? 3140}
      prefix="$"
      format="compact"
    />
  );
