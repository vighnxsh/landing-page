...
export const TokenChart: React.FC<ChartProps> = memo(({ renderingId, style, opt }) => {
      ...
      try {
        ...
        const widget = new tv.widget({
          ...
          overrides: {
            ...
            'mainSeriesProperties.candleStyle.upColor': '#ff69b4', // pink
            'mainSeriesProperties.candleStyle.downColor': '#ff69b4', // pink
            'mainSeriesProperties.candleStyle.borderUpColor': '#ff69b4', // pink
            'mainSeriesProperties.candleStyle.borderDownColor': '#ff69b4', // pink
            'mainSeriesProperties.candleStyle.wickUpColor': '#ff69b4', // pink
            'mainSeriesProperties.candleStyle.wickDownColor': '#ff69b4', // pink
            'mainSeriesProperties.lineStyle.color': '#ff69b4', // pink for line chart
            'mainSeriesProperties.lineWithMarkersStyle.color': '#ff69b4', // pink for line with markers
          },
        ...
        widget.onChartReady(() => {
          ...
          // Set chart style to 'Line with Markers' (style id 14) by default
          try {
            if (activeChart.chartType && activeChart.setChartType) {
              if (activeChart.chartType() !== 14) {
                activeChart.setChartType(14); // 14 = Line with Markers
              }
            }
          } catch (e) {
  ...
  if (!symbol) {
    return (
      <div className="flex items-center justify-center h-full text-red-600 bg-neutral-100 rounded-md">
        <span>Chart unavailable: Token information not found.</span>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div
        className={cn('relative h-full w-full flex-1 overflow-hidden transition-all border-2 border-[#111]')}
